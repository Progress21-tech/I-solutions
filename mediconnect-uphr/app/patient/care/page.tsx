...
type Provider = { id: string; clinician_id: string | null; provider_type: ProviderType; display_name: string; specialty: string | null; address: string; latitude: number; longitude: number }
...
function CareMarketplaceContent() {
  ...
  async function load() {
    ...
    const [{ data: patient, error: patientError }, { data: providerData, error: providerError }] = await Promise.all([
      supabase.from('patients').select('id, latitude, longitude, location_label').eq('user_id', auth.user.id).single(),
      supabase.from('providers').select('id, clinician_id, provider_type, display_name, specialty, address, latitude, longitude').eq('is_listed', true),
    ])
    if (patientError || !patient) setError('We could not load your saved location. You can still browse all listed care providers.')
    else { setPatientId(patient.id); setLocation(patient) }
  ...
  const requestTeleconsultation = async (provider: Provider) => {
    if (!patientId || !provider.clinician_id) return
    setError('')
    const { error: bookingError } = await supabase.from('teleconsultations').insert({ patient_id: patientId, clinician_id: provider.clinician_id, status: 'requested' })
    setError(bookingError ? 'We could not send the consultation request. Please try again.' : `Your consultation request has been sent to ${provider.display_name}.`)
  }
