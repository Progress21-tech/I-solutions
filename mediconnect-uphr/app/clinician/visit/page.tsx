    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans pb-16">
      ...
      <header className="border-b border-slate-800 bg-slate-950 px-6 py-4 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-3">
          <Link href="/clinician/dashboard" className="text-slate-400 hover:text-white text-sm font-semibold">
            ← Back to Dashboard
          </Link>
          <span className="text-slate-600">/</span>
          <h1 className="font-bold text-white text-base">Structured Clinical Visit Entry</h1>
        </div>

        <div className="flex items-center gap-3">
          <span className="text-xs text-slate-400">Patient: <strong className="text-white">{patient.full_name}</strong> ({patient.health_id})</span>
          <button
            onClick={handleSaveVisit}
            disabled={saving || savedSuccess}
            className={`font-bold text-xs px-5 py-2.5 rounded-xl transition shadow-lg flex items-center gap-2 ${
              savedSuccess
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <section className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-slate-950/80 border border-slate-800 rounded-3xl p-6 space-y-5">
            <div className="flex items-center justify-between border-b border-slate-800/80 pb-3">
              ...
              <span className="text-[11px] text-slate-400">Standard Antenatal & Cardiovascular Panel</span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2.5 text-white font-mono text-sm focus:border-emerald-500 focus:outline-none"
                />
              </div>

              <div>
                <label className="block text-slate-300 font-bold mb-1">
                  Diastolic BP <span className="text-slate-500 font-normal">(mmHg)</span>
                </label>
                <input
                  type="number"
                  value={diastolic}
              ...
              <div>
                <label className="block text-slate-300 font-bold mb-1">
                  Heart Rate <span className="text-slate-500 font-normal">(bpm)</span>
                </label>
                <input
                  type="number"
                  value={heartRate}
              ...
              <div>
                <label className="block text-slate-300 font-bold mb-1">
                  Maternal Weight <span className="text-slate-500 font-normal">(kg)</span>
                </label>
                <input
                  type="number"
                  step="0.1"
              ...
              {patient.pathway === 'maternal' && (
                <>
                  <div>
                    <label className="block text-slate-300 font-bold mb-1">
                      Gestational Age <span className="text-slate-500 font-normal">(wks)</span>
                    </label>
                    <input
                      type="number"
                  ...
                  <div>
                    <label className="block text-slate-300 font-bold mb-1">
                      Fundal Height <span className="text-slate-500 font-normal">(cm)</span>
                    </label>
                    <input
                      type="number"
                      value={fundalHeight}
                  ...
                  <div>
                    <label className="block text-slate-300 font-bold mb-1">
                      Fetal Heart Rate <span className="text-slate-500 font-normal">(bpm)</span>
                    </label>
                    <input
                      type="number"
              ...
              <div>
                <label className="block text-slate-300 font-bold mb-1">
                  Blood Glucose <span className="text-slate-500 font-normal">(mg/dL)</span>
                </label>
                <input
                  type="number"
                  value={bloodGlucose}
              ...
              <div>
                <label className="block text-slate-300 font-bold mb-1">
                  Hemoglobin / PCV <span className="text-slate-500 font-normal">(g/dL)</span>
                </label>
                <input
                  type="number"
                  step="0.1"
          ...
          {/* Notes & Diagnoses */}
          <div className="bg-slate-950/80 border border-slate-800 rounded-3xl p-6 space-y-4 text-xs">
            <h3 className="text-sm font-bold text-white flex items-center gap-2">
              <span>📋</span> Clinical Assessment & Diagnoses
            </h3>

            <div>
              <label className="block text-slate-300 font-bold mb-1">Diagnoses (comma-separated)</label>
              <input
                type="text"
                value={diagnoses}
            ...
            <div>
              <label className="block text-slate-300 font-bold mb-1">Clinical Notes & Observations</label>
              <textarea
                rows={3}
                value={clinicalNotes}
          ...
          <div className="bg-slate-950/80 border border-slate-800 rounded-3xl p-6 space-y-4 text-xs">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] text-emerald-400">Refill-enabled for Patient App</span>
            </div>
            ...
            <div className="space-y-2">
              {prescriptions.map((rx, idx) => (
                ...
                >
                  <div>
                    <span className="font-bold text-white">{rx.medication_name}</span>
            ...
            {/* Add Rx subform */}
            <form onSubmit={handleAddPrescription} className="pt-2 border-t border-slate-800 grid grid-cols-2 sm:grid-cols-4 gap-2">
              <input
                type="text"
                placeholder="Medication name"
                value={newMedName}
                onChange={(e) => setNewMedName(e.target.value)}
                className="bg-slate-900 border border-slate-700 rounded-xl px-2.5 py-1.5 text-white col-span-2"
              />
              <input
                type="text"
              ...
              <button
                type="submit"
                className="bg-emerald-700 hover:bg-emerald-600 text-white font-bold rounded-xl px-3 py-1.5 transition"
              >
                + Add Rx
              </button>
            </form>
        ...
        <section className="lg:col-span-5 space-y-5">
          <div className="bg-slate-950/90 border border-slate-800 rounded-3xl p-6 shadow-2xl sticky top-24 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
                <span className="text-[10px] uppercase tracking-wider font-extrabold text-emerald-400">
                  LIVE AI RISK CALCULATION
                </span>
                <h3 className="font-bold text-white text-base mt-0.5">Instant Clinical Score</h3>
              </div>
            ...
            <div>
              <div className="flex justify-between text-xs text-slate-400 mb-1">
                ...
                <span className="font-bold text-white">{liveRisk?.score}%</span>
              </div>
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {liveRisk?.drivingFactors.map((df, idx) => (
                  <div key={idx} className="text-xs bg-slate-900 p-2.5 rounded-xl border border-slate-800 text-slate-200">
                    • {df}
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {liveRisk?.clinicalRecommendations.map((rec, idx) => (
                  <div key={idx} className="text-xs bg-emerald-950/20 border border-emerald-800/30 p-2.5 rounded-xl text-emerald-200">
                    ✓ {rec}
                  </div>
                ))}
              </div>
            ...
            <button
              onClick={handleSaveVisit}
              disabled={saving || savedSuccess}
              className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3.5 rounded-2xl transition shadow-xl text-sm"
            >
...
export default function Page() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-900 grid place-items-center text-white">Loading visit entry...</div>}>
